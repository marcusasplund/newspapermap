...
function searchComplete(oSearch, sDivSearch) {
	var contentDiv = document.getElementById(sDivSearch);
	contentDiv.innerHTML = '<img src="logo-'+sDivSearch+'.png" class="logo" />';
...
function searchPeople(sKeywords) {
	...
	oSearchHitta = new google.search.WebSearch();
	oSearchHitta.setSiteRestriction('hitta.se');
	oSearchHitta.setSearchCompleteCallback(this, searchComplete, [oSearchHitta, 'resultsHitta']);
	oSearchHitta.setResultSetSize(google.search.Search.LARGE_RESULTSET);
	oSearchHitta.execute('"on Hitta" '+sKeywords);
}
